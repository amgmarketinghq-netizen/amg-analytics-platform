...
Main Application Entrypoint: app.py (v6.6 — Auto Pass Expiration & Full Security)
===================================================================================
Integrated SaaS Platform with High-Visibility White/Gold Banner, Corner Ghost Branding,
Rate-Limited Brute Force Lockout, SHA-256 Auth & Automatic 24h/48h/72h Pass Expiry.
"""
...
from utils.database import (
    init_db, add_client, toggle_client_status, delete_client, check_client_pass_status, 
...
if not st.session_state["auth_role"]:
    ...
    with col_l2:
        ...
        # CLIENT LOGIN FORM (WITH TIMED PASS EXPIRY CHECK)
            ...
            with st.form("main_client_login_form"):
                ...
                if client_submit:
                    if not client_email_input.strip() or not client_pass_input.strip():
                        ...
                        # Check Expiration & Status in Database
                        pass_status = check_client_pass_status(cleaned_email)

                        if pass_status == "NOT_FOUND":
                            st.error("❌ Access Denied! Invalid credentials or un-registered account.")
                        elif pass_status == "EXPIRED":
                            st.error("⌛ Access Pass Expired! Your timed pass duration has ended. Please contact AMG Admin to renew.")
                        elif pass_status == "INACTIVE":
                            ...
                            if client_record:
                                ...
                                st.error("❌ Access Denied! Account not found.")
