...
Main Application Entrypoint: app.py (v2.6 Enterprise Edition — High Contrast & Ghost Branding)
===================================================================================
Integrated SaaS Platform featuring High-Contrast Theme Banner, Ghost Watermarking,
Secrets-Driven RBAC & 20-Sector AI Detection.
"""
...
# Secret Admin Passcode (Protected lookup via Secrets)
try:
    ADMIN_PASSCODE = st.secrets.get("ADMIN_PASSCODE", "amg2026")
except Exception:
...
def generate_ghost_html_report(df, archetype_label):
    """Generates Standalone HTML Dashboard Report with Ghost AMG Watermark in Bottom Right Corner."""
    table_rows = ""
    ...
    <head>
        ...
        <style>
            ...
            .header {{ background: linear-gradient(90deg, #1A1F2B 0%, #0B4F6C 100%); padding: 25px; border-radius: 12px; border: 1px solid #2A3140; text-align: center; }}
            h1 {{ color: #FFF; margin: 0; font-size: 24px; }}
            .sub {{ color: #57BB8A; font-weight: bold; margin-top: 8px; font-size: 14px; }}
            .card {{ background: #1A1F2B; padding: 20px; border-radius: 10px; border: 1px solid #2A3140; margin-top: 20px; }}
            table {{ width: 100%; border-collapse: collapse; margin-top: 15px; color: #E2E8F0; }}
            th, td {{ padding: 10px; border-bottom: 1px solid #2A3140; text-align: left; font-size: 13px; }}
            th {{ background-color: #0B4F6C; color: #FFF; }}

            /* EMBOSSED GHOST WATERMARK IN BOTTOM RIGHT CORNER */
            .ghost-watermark {{
                position: fixed;
                bottom: 8px;
                right: 14px;
    ...
    <body>
        ...
        <!-- HIDDEN BRANDING WATERMARK -->
        <div class="ghost-watermark">AMG</div>
    </body>
    </html>
    """
...
# ==========================================
# 1. MAIN SCREEN AUTHENTICATION (HIGH CONTRAST BANNER)
# ==========================================
if not st.session_state["auth_role"]:
    # SOLID DARK GRADIENT CARD BANNER (PREVENTS WHITE-ON-WHITE INVISIBILITY)
    st.markdown("""
    <div style="background: linear-gradient(135deg, #0B4F6C 0%, #1A1F2B 100%); border: 1px solid #2A3140; border-radius: 16px; padding: 30px; text-align: center; margin-bottom: 25px; box-shadow: 0 8px 24px rgba(0,0,0,0.3);">
        <img src="https://img.icons8.com/color/96/combo-chart.png" width="65"/>
        <h1 style="color: #FFFFFF !important; font-size: 30px; font-weight: 800; margin: 12px 0 4px 0; letter-spacing: 0.5px;">
            AMG Enterprise Analytics Portal
        </h1>
        <p style="color: #57BB8A !important; font-size: 15px; font-weight: 600; margin: 0;">
            Data Infrastructure & Strategic Decision Command Center
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_l2:
        ...
        with tab_admin_login:
            ...
            with st.form("main_admin_login_form"):
                ...
                if admin_submit:
                    if admin_pass_input == ADMIN_PASSCODE:
...
# ==========================================
st.markdown(f"""
<div style="background-color: #1A1F2B; border: 1px solid #2A3140; padding: 12px 24px; border-radius: 12px; margin-bottom: 20px;">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
            ...
            <span style="color: #2A3140; margin: 0 10px;">|</span>
            <span style="color: #94A3B8; font-size: 13px;">Account:</span> 
            <b style="color: #57BB8A; font-size: 15px;">{st.session_state['user_identity']}</b>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
if admin_menu == "📊 Data Analytics Dashboard":
    ...
    if uploaded_file is not None:
        try:
            ...
            # HTML GHOST BRANDING REPORT DOWNLOAD BUTTON
            st.divider()
...
# ==========================================
# 5. ADMIN FEATURE: MANAGE CLIENT ACCESS (WITH 24H/48H/72H/PERMANENT PASSES)
# ==========================================
