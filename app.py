...
Main Application Entrypoint: app.py (v6.0 — Fully Wired Hashed Passcode Security)
===================================================================================
Integrated SaaS Platform with High-Visibility White/Gold Banner, Corner Ghost Branding,
Rate-Limited Brute Force Lockout, SHA-256 Client PIN Verification & Global Tax Controls.
"""
        ...
        if st.button("Activate & Grant Pass", use_container_width=True):
            if c_name and c_email and c_passcode:
                if add_client(c_name, c_email, c_status, passcode=c_passcode):
                    st.success(f"✅ Client '{esc(c_name)}' registered with PIN '{esc(c_passcode)}' & {pass_duration} Access!")
                    st.rerun()
                else:
