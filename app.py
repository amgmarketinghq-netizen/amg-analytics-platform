...
===================================================================================
Integrated Multi-Dashboard SaaS Platform with Auto-Archetype Detection,
Client Access Management, Invoice Generator, and Audit Log Clear.
"""
...
from utils.database import (
    ...
    get_all_clients, log_audit_trail, get_audit_logs, clear_audit_logs, create_invoice, get_all_invoices
)
...
if app_mode == "📊 Live Analytics Engine":
    ...
    if client_email_input:
        is_active = is_client_active(client_email_input)
        if not is_active:
            st.error(f"⛔ Access Denied for `{client_email_input}`. Account is INACTIVE or Unregistered.")
            st.info("Please contact AMG Billing to activate your portal access.")
            st.stop()
        else:
    ...
    if uploaded_file is not None:
        ...
        st.write("Upload a CSV file in the sidebar to begin 100% accurate mathematical analytics & data sanitation.")


# ==========================================
# MODE 2: CLIENT ACCESS CONTROL (ADMIN)
# ==========================================
...
# ==========================================
    ...
    with col_a:
        ...
        st.markdown("---")
        service_desc = st.text_input("Service Description", value="Data Sanitation & Deduplication Pilot Batch")
...
# ==========================================
# MODE 4: AUDIT TRAIL LOGS (WITH DELETE BUTTON)
# ==========================================
    ...
    if logs:
        ...
        with c_log1:
            st.subheader("System Processing History")
        with c_log2:
