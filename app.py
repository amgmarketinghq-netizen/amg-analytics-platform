...
Main Application Entrypoint: app.py (v3.3 — Invoice Date, Flexible Global Tax & High-Contrast Edition)
===================================================================================
Integrated SaaS Platform with High-Visibility White/Gold Banner, Corner Ghost Branding,
Encrypted Secure RBAC, 20-Sector AI Detection & Dynamic Global Tax/GST Invoicing.
"""
...
# ==========================================
# 6. ADMIN FEATURE: INVOICE GENERATOR (DYNAMIC GLOBAL TAX & DATE COMPLIANCE)
# ==========================================
    ...
    with col_a:
        ...
        st.markdown("---")
        # DYNAMIC GLOBAL TAX & GST COMPLIANCE
        st.subheader("🌐 Global Tax / GST Compliance (Optional)")
        include_gst = st.checkbox("Include Tax / GSTIN Details (For Business Verification)")
        ...
        if include_gst:
            ...
            with g_col1:
                agency_gst_input = st.text_input("Agency Tax ID / GSTIN:", value="20AAAAA0000A1Z5", help="Your business Tax ID")
            with g_col2:
                client_gst_input = st.text_input("Client Tax ID / GSTIN (Optional):", placeholder="Client Tax ID")
            tax_label_name = st.text_input("Tax Name Label:", value="GST", help="e.g. GST, VAT, Sales Tax")

        st.markdown("---")
        st.subheader("💼 Service Details & Flexible Tax Rate")
        ...
        # FULLY EDITABLE TAX RATE % (Supports 0%, 5%, 12%, 18%, 20% or any custom value)
        tax_pct = st.number_input(
            f"Custom Tax / {tax_label_name} Rate (%)", 
            value=18.0 if include_gst else 0.0, 
            step=1.0, 
            min_value=0.0, 
            max_value=100.0
        )
    ...
    with col_b:
        ...
        # Prepare dynamic agency & client names with Tax ID + Date Stamp
