...
Main Application Entrypoint: app.py (v6.2 — Client Remove Button & Full Security Edition)
===================================================================================
Integrated SaaS Platform with High-Visibility White/Gold Banner, Corner Ghost Branding,
Rate-Limited Brute Force Lockout, SHA-256 Client PIN Verification & Client Removal.
"""
...
# Security State Initialization (Rate Limiting & Lockout)
...
# ==========================================
# 5. ADMIN FEATURE: MANAGE CLIENT ACCESS (WITH DELETE BUTTON)
# ==========================================
    ...
    with col1:
        ...
        auto_gen_pin = secrets.token_hex(4)
        c_passcode = st.text_input("Client Access Passcode / PIN", value=auto_gen_pin, help="Unique generated PIN for client login")
        ...
        if st.button("Activate & Grant Pass", use_container_width=True):
            if c_name and c_email and c_passcode:
                if add_client(c_name, c_email, c_status, passcode=c_passcode):
                    st.success(f"✅ Client '{esc(c_name)}' registered/updated with PIN '{esc(c_passcode)}' & {pass_duration} Access!")
                    st.rerun()
                else:
                    st.error("Error processing client registration.")
            else:
        ...
        if clients:
            ...
            if filtered_clients:
                for c in filtered_clients:
                    ...
                    st.write(f"**{esc(c['client_name'])}** (`{esc(c['email'])}`) | Status: **{status_badge}**")
                    
                    # ACTION BUTTONS: SWITCH STATUS & REMOVE CLIENT
