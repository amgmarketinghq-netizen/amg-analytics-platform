...
Main Application Entrypoint: app.py (v6.5 — PIN Overwrite Bug Fix & Full Security)
===================================================================================
Integrated SaaS Platform with High-Visibility White/Gold Banner, Corner Ghost Branding,
Rate-Limited Brute Force Lockout, Session-Safe SHA-256 Client PIN Auth & Deletion.
"""
...
# Stable Random PIN Generator (Prevents dynamic overwrite bug)
if "default_gen_pin" not in st.session_state:
    st.session_state["default_gen_pin"] = secrets.token_hex(4)


# ==========================================
# 1. MAIN SCREEN AUTHENTICATION WITH RATE-LIMIT LOCKOUT
# ==========================================
if not st.session_state["auth_role"]:
    ...
    with col_l2:
        ...
        # CLIENT LOGIN FORM (SHA-256 HASH VERIFICATION)
        with tab_client_login:
            ...
            with st.form("main_client_login_form"):
                ...
                if client_submit:
                    if not client_email_input.strip() or not client_pass_input.strip():
                        ...
                        if not client_record:
                            st.error("❌ Access Denied! Invalid credentials or un-registered account.")
                        elif client_record["status"].lower() != "active":
                            st.warning("⚠️ Account Suspended! Access for this account is INACTIVE.")
                        else:
                            # Strict SHA-256 Hash Passcode Verification
        ...
        # ADMIN LOGIN FORM (WITH BRUTE FORCE RATE-LIMITING)
        with tab_admin_login:
            ...
            if st.session_state["admin_lockout_until"]:
                ...
                if now < st.session_state["admin_lockout_until"]:
                    remaining_sec = int((st.session_state["admin_lockout_until"] - now).total_seconds())
                    st.error(f"⛔ Too many failed attempts! Account locked for safety. Try again in {remaining_sec} seconds.")
                    st.stop()
                else:
                    st.session_state["admin_failed_attempts"] = 0
                    st.session_state["admin_lockout_until"] = None
                ...
                if admin_submit:
                    if verify_admin_passcode(admin_pass_input):
                        ...
                        if st.session_state["admin_failed_attempts"] >= 5:
                            st.session_state["admin_lockout_until"] = dt.datetime.now() + dt.timedelta(minutes=5)
                            st.error("⛔ Account locked for 5 minutes due to 5 consecutive failed attempts!")
                        else:
                            tries_left = 5 - st.session_state["admin_failed_attempts"]
                            st.error(f"❌ Invalid Admin Passcode! ({tries_left} attempts remaining before lockout)")
...
if admin_menu == "📊 Data Analytics Dashboard":
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            st.error("⚠️ Error processing dataset. Please check your file format and ensure valid structure.")
    else:
        st.info("👆 Please upload a CSV or Excel dataset above to render live analytics.")


# ==========================================
# 5. ADMIN FEATURE: MANAGE CLIENT ACCESS (SAFE PIN FIX)
# ==========================================
    ...
    with col1:
        ...
        with st.form("client_registration_form"):
            ...
            # Text Input with Session-Safe Default (Does not overwrite typed text)
            c_passcode = st.text_input(
                "Client Access Passcode / PIN", 
                value=st.session_state["default_gen_pin"], 
                help="Type custom password (e.g. Haidar02) or use auto PIN"
            )
            ...
            if submit_reg:
                if c_name.strip() and c_email.strip() and c_passcode.strip():
                    if add_client(c_name, c_email, c_status, passcode=c_passcode):
                        # Generate fresh PIN for next registration
                        st.session_state["default_gen_pin"] = secrets.token_hex(4)
                        st.success(f"✅ Passcode for '{esc(c_name)}' successfully set to: **`{esc(c_passcode.strip())}`**")
                        st.rerun()
                    else:
        ...
        if clients:
            ...
            if filtered_clients:
                for c in filtered_clients:
                    ...
                    has_pin = "🔒 Passcode Set" if c.get("passcode_hash") else "⚠️ No Passcode"
                    
                    st.write(f"**{esc(c['client_name'])}** (`{esc(c['email'])}`)")
                    st.caption(f"Status: **{status_badge}** | Key Status: **{has_pin}**")
