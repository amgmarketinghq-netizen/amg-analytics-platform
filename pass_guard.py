...
if "client_access_db" not in st.session_state:
    st.session_state["client_access_db"] = {
        "mdhaidara451@gmail.com": {
            ...
            "is_active": True,
            "label": "Haidar Personal VIP Account"
        }
    }
...
def render_admin_client_manager():
    ...
    for email, record in list(db.items()):
        ...
        dur_label = f"{record['hours']}h" if record.get('hours') else "Permanent"
...
# DIRECT ALIAS FUNCTIONS (No self-imports)
...
def render_lock_screen():
    st.markdown("""
    <div style="background-color: #1A1F2B; border: 2px solid #E06666; padding: 30px; border-radius: 12px; text-align: center; margin-top: 30px;">
        <h2 style="color: #E06666; margin-bottom: 10px;">🔒 ACCESS RESTRICTED</h2>
        <p style="color: #CBD5E1; font-size: 15px;">
            Please enter your registered <b>Email ID</b> in the sidebar to unlock access.
        </p>
    </div>
