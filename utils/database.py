...
Database Utility Layer: utils/database.py (v6.2 — Added Client Deletion)
===================================================================================
Handles Persistent Storage for Clients, Hashed Passcodes, Deletion & Audit Trails.
"""
...
def add_client(client_name: str, email: str, status: str = "Active", passcode: str = None) -> bool:
    """Adds a new client record OR updates existing client's passcode/status seamlessly."""
    if not passcode:
    ...
    try:
        ...
        if existing:
            # Update existing client's passcode_hash, name, and status
            cursor.execute(
                "UPDATE clients SET client_name = ?, status = ?, passcode_hash = ? WHERE LOWER(email) = ?",
                (client_name.strip(), status, pass_hash, clean_email)
            )
        else:
            # Insert brand new client
            cursor.execute(
                "INSERT INTO clients (client_name, email, status, passcode_hash) VALUES (?, ?, ?, ?)",
                (client_name.strip(), clean_email, status, pass_hash)
            )
