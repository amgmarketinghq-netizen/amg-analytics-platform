...
Includes Standalone Offline-Ready Plotly HTML Export, Dynamic Branding & Custom Charts.
"""
...
def generate_full_html_report(df: pd.DataFrame, cleaned_df: pd.DataFrame, fig_custom, fig_pie, raw_total: int, dup_count: int, missing_email: int, clean_total: int, logo_url: str = "") -> str:
    """Generates 100% offline-ready HTML report with embedded Plotly JS for mobile viewing."""
    ...
    # Embed Plotly JS inline so mobile viewers don't block CDN network requests
    custom_chart_html = fig_custom.to_html(full_html=False, include_plotlyjs=True) if fig_custom else "<p>No custom chart available</p>"
    pie_chart_html = fig_pie.to_html(full_html=False, include_plotlyjs=False) if fig_pie else "<p>No health chart available</p>"
...
<head>
    ...
    <style>
        ...
        .chart-box {{ flex: 1; min-width: 320px; background: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); }}
