...
Includes Base64 Local Image Upload (Mobile/Laptop Gallery), Fixed Height Chart Layouts & Visible Ghost Watermark.
"""
...
def image_to_base64(uploaded_file):
    """Converts local uploaded image file to Base64 string for direct HTML embedding."""
...
def generate_full_html_report(df: pd.DataFrame, cleaned_df: pd.DataFrame, fig_custom, fig_pie, raw_total: int, dup_count: int, missing_email: int, clean_total: int, logo_src: str = "") -> str:
    """Generates 100% responsive HTML report with embedded Base64 logo and subtle AMG watermark."""
    ...
    # Seamless Logo HTML (Base64 or URL support)
    logo_header_html = f'<div style="margin-bottom:15px;"><img src="{logo_src}" style="max-height: 55px; width: auto; display: block;" alt="Client Logo"></div>' if logo_src.strip() else ''
...
<head>
    ...
    <style>
        ...
        /* Subtle AMG Ghost Watermark (Faint Grey, Readable on Gaur Looking) */
        .stealth-watermark {{ 
            text-align: center; 
            margin-top: 50px; 
            padding: 15px; 
...
def render_customer_behavior(df: pd.DataFrame, col_map: dict):
    """Renders B2B Leads Analytics & Raw Audit Engine with Local File Branding & Custom Charts."""
    ...
    # ------------------------------------------
    # CUSTOMER BRANDING (LOCAL FILE UPLOAD + URL FALLBACK) & CHART CONTROLS
    # ------------------------------------------
    ...
    with col_brand:
        uploaded_logo = st.file_uploader(
            "🖼️ Upload Client Logo (PNG/JPG from Mobile/Laptop):",
            type=["png", "jpg", "jpeg"]
        )
    ...
    if uploaded_logo is not None:
        final_logo_src = image_to_base64(uploaded_logo)
    elif custom_logo_url.strip():
        final_logo_src = custom_logo_url.strip()
